...
$users = "schopade@aptean.com" # List of users to email your report to (separate by comma)
$fromemail = "NoReply-Apprise@aptean.com"
$server = "smtp.office365.com" #enter your own SMTP server DNS name / IP address here
$list = "F:\Temp\list.txt" #$args[0] #This accepts the argument you add to your scheduled task for the list of servers. i.e. list.txt
$pwd = (ConvertTo-SecureString -AsPlainText -String Winter@123 -Force)
$cred=New-Object System.Management.Automation.PSCredential($fromemail,$pwd)
...
if ($regex.IsMatch($regexsubject)) {
   send-mailmessage -from $fromemail -Credential $cred -to $users -subject "Disk Space Monitoring Report" -BodyAsHTML -body $HTMLmessage -priority High -smtpServer $server -Port 587 -UseSsl
}
